...
class LineAnnotation(BaseModel):
    path: str = Field(description="Relative path of the audited file, e.g., 'vulnerable_sample.py'")
...
class PRReviewPayload(BaseModel):
    ...
    refactored_code: Optional[str] = Field(
        default=None, 
        description="The complete, fully corrected source code with all vulnerabilities, SQL injections, and hardcoded secrets fixed."
    )